...
=head2 Plugin C<notify_flapjack>

The I<notify_flapjack> plugin writes notifications to Flapjacks I<redis database> directly
to the I<events> queue.

Synopsis:

  <Plugin "notify_flapjack">
    <Node "example">
        Host "localhost"
        Port "6379"
        Timeout 1000
...
Notifications are pushed to the I<events queue>, just as Flapjack expects them.
You can specify the database to use with the B<Database> parameter (default is C<0>). 
See L<http://flapjack.io/> for reference.
